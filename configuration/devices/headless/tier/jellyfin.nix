      ...
      options = [
        ...
        "map=${
          lib.concatStringsSep ":" [
            "${config.services.syncthing.user}/${config.services.jellyfin.user}"
            "@${config.services.syncthing.group}/@${config.services.jellyfin.group}"
          ]
        }"
      ];
    };
  ...
  systemd.services.jellyfin-setup = {
    ...
    script = ''
      ...
      if [ -f "$mediaBarFile" ]; then
        old_hash="$(sha256sum "$mediaBarFile" | ${pkgs.gawk}/bin/awk '{print $1}')"
        ...
        new_hash="$(sha256sum "$mediaBarFile" | ${pkgs.gawk}/bin/awk '{print $1}')"
      ...
      if [ -f "$ssoAuthFile" ]; then
        old_hash="$(sha256sum "$ssoAuthFile" | ${pkgs.gawk}/bin/awk '{print $1}')"
        ...
        new_hash="$(sha256sum "$ssoAuthFile" | ${pkgs.gawk}/bin/awk '{print $1}')"
