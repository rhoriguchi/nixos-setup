      ...
      lib.nameValuePair "couchdb-init-${database}" {
        ...
        script = ''
          ...
          if [ "$status" -eq 404 ]; then
            ${pkgs.curl}/bin/curl -s -X PUT -u "admin:${config.services.couchdb.adminPass}" http://127.0.0.1:${toString config.services.couchdb.port}/${database}
          fi

          ${pkgs.curl}/bin/curl -s -X PUT -u "$admin_auth" \
            -H "Content-Type: application/json" \
            -d '{"admins":{"names":["${database}"],"roles":[]},"members":{"names":["${database}"],"roles":[]}}' \
            http://127.0.0.1:${toString config.services.couchdb.port}/${database}/_security
        '';
