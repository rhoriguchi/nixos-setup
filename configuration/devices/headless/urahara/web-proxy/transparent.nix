  ...
  systemd.services.nginx-transparent = {
    ...
    script = ''
      ...
      ${lib.optionalString config.networking.enableIPv6 ''
        ip -6 rule del fwmark ${mark} lookup nginx-transparent 2>/dev/null || true
        ip -6 rule add fwmark ${mark} lookup nginx-transparent
        ip -6 route replace local ::/0 dev lo table nginx-transparent
      ''}
    '';

    preStop = ''
      ${lib.optionalString config.networking.enableIPv6 ''
        ip -6 route del local ::/0 dev lo table nginx-transparent 2>/dev/null || true
        ip -6 rule del fwmark ${mark} lookup nginx-transparent 2>/dev/null || true
      ''}
