...
in {
  services.home-assistant.config.automation = [
    ...
    {
      alias = "Reset office Signe Gradient window when turned on";
      triggers = [{
        trigger = "state";
        entity_id = "light.office_signe_gradient_window";
        from = "off";
        to = "on";
      }];
      actions = [{
        action = "light.turn_on";
        target.entity_id = "light.office_signe_gradient_window";
        data.color_temp_kelvin = 3600;
      }];
    }
