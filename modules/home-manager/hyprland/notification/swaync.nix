...
let
  ...
  configFile = pkgs.writeTextFile rec {
    ...
    text = builtins.readFile (settingsFormat.generate name settings);
  ...
  config = lib.mkIf cfg.enable {
    ...
    xdg.configFile = {
      ...
      "swaync/style.css" = lib.mkIf (cfg.style != null) {
        source = if builtins.isPath cfg.style || lib.isStorePath cfg.style then cfg.style else pkgs.writeText "swaync/style.css" cfg.style;
      };
    };
    ...
    systemd.user.services.swaync = lib.mkIf cfg.systemd.enable {
      Unit = {
        ...
        X-Restart-Triggers = [ "${config.xdg.configFile."swaync/config.json".source}" ]
