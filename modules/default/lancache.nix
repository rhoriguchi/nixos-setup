...
let
  ...
  rawCacheDomains = lib.flatten (
    map (domainFile: (lib.splitString "\n" (builtins.readFile "${src}/${domainFile}"))) domainFiles
  );
...
in
{
  options.services.lancache = {
    ...
    cachedServices =
      let
        services = map (cacheDomain: cacheDomain.name) metadata;
      in
      lib.mkOption {
  ...
  config = lib.mkIf cfg.enable {
    ...
    virtualisation.oci-containers.containers.lancache = {
      ...
      environment = rec {
        ...
        CACHE_INDEX_SIZE = "${
          toString (lib.toInt (lib.head (lib.splitString "g" CACHE_DISK_SIZE)) / 1000 * 250)
        }m";
      ...
      ports = [
        ...
        "127.0.0.1:${toString cfg.statusPort}:8080"
      ];

      volumes = [
        "${cfg.cacheDir}:/data/cache"
        "/var/log/lancach:/data/logs"
      ];
    };
