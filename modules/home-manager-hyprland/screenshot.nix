...
let
  ...
  regionScreenshotScript = pkgs.writeShellScript "screenshot-region.sh" ''
    ${pkgs.procps}/bin/pkill -u "$USER" -f hyprshot

    ${config.programs.hyprshot.package}/bin/hyprshot --mode region
  '';
in {
  programs.hyprshot = {
    enable = true;
  ...
  wayland.windowManager.hyprland.settings.bind = [
    # TODO HYPRLAND combine with swappy there's a home manager module https://github.com/Gustash/Hyprshot/issues/106
