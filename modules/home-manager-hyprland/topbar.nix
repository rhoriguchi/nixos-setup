...
let disabledColor = colors.extra.terminal.border;
in {
  ...
  programs.waybar = {
    ...
    settings.topBar = {
      ...
      modules-right = [ "network#traffic" "cpu" "memory" "disk" "idle_inhibitor" "custom/notification" "tray" ];
    ...
    style = ''
      * {
      ...
      ${
        let
          configuredModules = config.programs.waybar.settings.topBar.modules-left ++ config.programs.waybar.settings.topBar.modules-center
            ++ config.programs.waybar.settings.topBar.modules-right;
          modules = map (module: lib.replaceStrings [ "hyprland/" "/" "#" ] [ "" "-" "." ] module) configuredModules;
        in lib.concatStringsSep ", " (map (module: "#${module}") modules)
      } {
        padding: 0 10px;
      }
    '';
