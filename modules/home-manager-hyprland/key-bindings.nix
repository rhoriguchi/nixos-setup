...
let
  ...
  activeMonitor =
    "${config.wayland.windowManager.hyprland.package}/bin/hyprctl monitors -j | ${pkgs.jq}/bin/jq -r '.[] | select(.focused == true).name'";
  swayosd-client = ''${config.services.swayosd.package}/bin/swayosd-client --monitor "$(${activeMonitor})"'';
in {
  services.swayosd = {
    enable = true;
  ...
  wayland.windowManager.hyprland = {
    settings = {
      ...
      gesture = [ "3, horizontal, workspace" "3, swipe, mod: $mainMod, resize" ];
    };
    ...
    submaps = {
      move.settings = {
        ...
        # e	repeat	Will repeat when held.
        binde = [ ", left, moveactive, -10 0" ", right, moveactive, 10 0" ", up, moveactive, 0 -10" ", down, moveactive, 0 10" ];
      };
      ...
      resize.settings = {
        ...
        # e	repeat	Will repeat when held.
        binde = [ ", left, resizeactive, -10 0" ", right, resizeactive, 10 0" ", up, resizeactive, 0 -10" ", down, resizeactive, 0 10" ];
      };
    };
  };
}
